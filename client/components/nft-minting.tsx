...
export function NFTMinting({ documentId, documentName, trustScore, onComplete }: NFTMintingProps) {
  ...
  useEffect(() => {
    if (status === "preparing") {
      // Start minting after a short delay
      ...
      const timer = setTimeout(() => {
        ...
        if (Math.random() > 0.05) {
          ...
          setNftData({
            ...
            certificateUrl: `/certificates/${documentId}`,
          })
        } else {
          setStatus("failed")
          setError("Transaction failed due to network congestion. Please try again.")
        }
      }, 3000)
      return () => clearTimeout(timer)
    }
    ...
    <GlassCard className="w-full">
      ...
      {status === "preparing" && (
        <div className="text-center py-8">
          ...
          <RippleButton onClick={startMinting} className="mx-auto">
